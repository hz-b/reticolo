...
title(sprintf('Bragg Energy Sweep | %d l/mm | Cr/C ×%d | %s | Order %+d', ...
    grPeriod_lpermm, num_multilayers, pol_str, options.GR_Order), 'FontSize', 11);

grid on;
set(gca, 'FontSize', 11);
xlim([min(results.sweep_values), max(results.sweep_values)]);

saveas(gcf, fullfile(options.output_dir, 'efficiency_bragg_sweep.png'));
fprintf('Plot saved.\n');
