...
for sv = sweep_values
    ...
    n_sub = load_cxro(resolve_oc(substrate_file, opt.oc_path), photon_eV);
    if isnan(real(n_sub))
        if opt.verbose
    ...
    for li = 1:numel(stack.layers)
        n_layers{li} = load_cxro(resolve_oc(stack.layers{li}.material_file, opt.oc_path), photon_eV);
...
function fp = resolve_oc(filename, oc_path)
% Prepend oc_path to filename if the file is not already an absolute path
% and does not exist as given. Lets callers pass bare names like 'n_Si_cxro.txt'
% while the optical constants live in a dedicated folder.
    if isempty(oc_path) || exist(filename, 'file') == 2
        fp = filename;
    else
        fp = fullfile(oc_path, filename);
    end
end
