...
%
%  [e,yy,ww,o,s,sssh]=retchamp(init,a,sh,sb,inc,x,tab,sssh,cale,calyy,calww,calo,cals,met);
%
...
%
%  [e,y,wy,o,s,sssh]=retchamp(init,a,sh,sb,inc,x,tab,   sssh,cale,calyy,calww,calo,cals,met);% forme generale conseillee 1D
%  [e,z,wz,o,s,sssh]=retchamp(init,a,sh,sb,inc,{x,y},tab,   sssh,cale,calyy,calww,calo,cals,met);% forme generale conseillee 2D
%   e=retchamp(init,a,inc,sens,x); % forme simplifiee une seule couche un seul plan
%   e=retchamp(init,sh,sb,inc); % autre forme simplifiee donnant le champ dans un plan
%
% pour memoire :rettchamp permet un trace automatique
%  en 1D  rettchamp(e,o,x,y,pol,[1:3,12,i]   ,parm,fig,Text); 
%  en 2D  rettchamp(e,o,x,y,z,[1:6,12,i]   ,parm,fig,Text); 
%
...
if iscell(init) & length(init)==2;
switch(nargin);
case 7;[e,yy,ww,o,s]=retyeh_GNV('retchamp',init,a,sh,sb,inc,x,y);
case 8;[e,yy,ww,o,s]=retyeh_GNV('retchamp',init,a,sh,sb,inc,x,y,h);
case 9;[e,yy,ww,o,s]=retyeh_GNV('retchamp',init,a,sh,sb,inc,x,y,h,cale);
case 10;[e,yy,ww,o,s]=retyeh_GNV('retchamp',init,a,sh,sb,inc,x,y,h,cale,calyy);
case 11;[e,yy,ww,o,s]=retyeh_GNV('retchamp',init,a,sh,sb,inc,x,y,h,cale,calyy,calww);
case 12;[e,yy,ww,o,s]=retyeh_GNV('retchamp',init,a,sh,sb,inc,x,y,h,cale,calyy,calww,calo);
case 13;[e,yy,ww,o,s]=retyeh_GNV('retchamp',init,a,sh,sb,inc,x,y,h,cale,calyy,calww,calo,cals);
end;
return;end;
...
if ischar(init{end});if nargin==7;[e,yy,ww,o]=retchamp_NonLocal(init,a,sh,sb,inc,x,y);
	else;[e,yy,ww,o]=retchamp_NonLocal(init,a,sh,sb,inc,x);end;
	return;end;% cas 0 D 'rapide' non local
...
for ii=nn-1:-1:double(nn>1); %àààààààà% modif 18 8 2015 double(nn>1) au lieu de 1
...
if calee & ii>0 ;sssh{ii}=retio(ssh,1);end;
...
if iscell(x);oooo=zeros(length(yyy),length(x{1}),length(x{2}),length(calo));else;oooo=zeros(length(yyy),length(x),length(calo));end;
...
if aprv{end}.type==4;  % elements finis
    global yy_global;if ~isempty(yy_global);yyy=yy_global;end;
...
non_accelere= (size(y,2)<4)|(min(abs(a{5}))<eps);
if  non_accelere;  %<-- !!!!forme non acceleree  modif 2017 
...
else;           %<-- !!!!  forme acceleree  modif 2017
...
Vm=zeros(2*m,length(y));
Vp=zeros(2*m,length(y));
if ~isempty(f);Vp(f,:)=exp(-a{5}(f)*y);Vm(m+f,:)=exp(a{5}(f)*(y-h));end;% sinon bug modif 22 9 2017
if ~isempty(ff);Vp(m+ff,:)=exp(a{5}(ff)*y);Vm(ff,:)=exp(-a{5}(ff)*(y-h));end;
...
Vm=Vm+Vp;
Vp=-a{1}* (retdiag(a{5})*(Vm(1:m,:)-Vm(m+1:2*m,:)));
Vm=a{3}*(Vm(1:m,:)+Vm(m+1:2*m,:));
V=[Vm;Vp];clear Vm Vp;


if ~isempty(a3);e(:,:,3)=retff((a3*V(1:m,:)).',betaa);end;       % hy
if ~isempty(a1);e(:,:,4)=retff((-i*a1*V(m+1:2*m,:)).',betaa);end;% bx
if ~isempty(si);V=[si{1}*V(1:m,:);si{1}*V(m+1:2*m,:)];end;
e(:,:,1)=retff(V(1:n,:).',betaa);%ez
if ~isempty(a1);e(:,:,2)=e(:,:,4)*retdiag(1./muy);else;e(:,:,2)=retff((-i*V(n+1:2*n,:)).',betaa);end;% 'meilleur' calcul de hx
...
e(nn,:,1)=retff(v(1:n).',betaa);%Ez
    ...
    if non_accelere; %<--!!!! modif 2017
if ~isempty(a3);e(nn,:,3)=retff((a3*v(1:m)).',betaa);end;       % hy
if ~isempty(a1);e(nn,:,4)=retff((-i*a1*v(m+1:2*m)).',betaa);end;% bx
if ~isempty(si);v=[si{1}*v(1:m);si{1}*v(m+1:2*m)];end;
e(nn,:,1)=retff(v(1:n).',betaa);%ez
if ~isempty(a1);e(nn,:,2)=e(nn,:,4)./(muy.');else;e(nn,:,2)=retff((-i*v(n+1:2*n)).',betaa);end;% 'meilleur' calcul de hx;
    end;              %<--!!!! Fin modif 2017
end;                % texture inclinee  ?
...
if size(uu,3)==18;% anisotropie generale 2015
for iii=1:3;for jjj=1:3;ep(1,fx,fy,iii,jjj)=uu(ii,jj,9+iii+3*(jjj-1));mu(1,fx,fy,iii,jjj)=uu(ii,jj,iii+3*(jjj-1));end;end	
else;
...
[prv1,prv2,prv3,prv4,T,prv5,prv6,E,y,w,o]=retabeles(init{1},tab(:,2),tab(:,1),init{2},init{3},[],init{4},npts);

% calcul du champ en bas
Th=retss(Th,T);Tb=retsp(Tb,-1);
Th(1,:)=Tb(1,:);% [ib,ih]=Th*[u1,u2];
Th=retsp(Th,-1);% [u1,u2]=Th*[ib,ih];
...
if init{4}==2;beta=init{2};% modif 8 2019
else;
beta=repmat(init{2},length(init{3}),1);
if init{4}==1;beta=retdiag(init{3})*beta;end; % Modif 18_2_2017
end;


e(:,:,1)=E(:,:,1,1).*repmat(Eb(:,1),1,sz(3))+E(:,:,1,2).*repmat(Eb(:,2),1,sz(3));
e(:,:,2)=-1i*(E(:,:,2,1).*repmat(Eb(:,1),1,sz(3))+E(:,:,2,2).*repmat(Eb(:,2),1,sz(3)));% -i declonage
% e(:,:,3)=-e(:,:,1).*(beta(:)*(1./((o.').^init{1})));

%ep=retep_nvarie(o,init{1},init{3});k0_s_mmu=retdiag(init{3})*reshape(1./ep(2,:,:),sz(3),sz(1)).';% modif 8 2015
ep=retep_nvarie(o,init{1},init{3});k0_s_mmu=reshape(1./ep(2,:,:),sz(3),sz(1)).';% modif 8 2015  % MODIF k0********

if init{4}==2;% modif 8 2019
e(:,:,3)=retdiag(-beta(:))*(repmat(k0_s_mmu,size(beta,2),1)).*e(:,:,1);% non cloné
else;
e(:,:,3)=retdiag(-beta(:))*(repmat(k0_s_mmu,length(init{2}),1)).*e(:,:,1);% non cloné
end;
% 
% global retabeles_change_indices retabeles_change_indices_bis;% modif 8 2015
% if init{1}==0;e(:,:,3)=retdiag(-beta(:))*e(:,:,1);
% else;
% if isempty(retabeles_change_indices);e(:,:,3)=-e(:,:,1).*(beta(:)*(1./((o.').^2)));
% else;
% if isempty(retabeles_change_indices_bis);
% oo=retabeles_change_indices(init{3},o);
% else;oo=retabeles_change_indices_bis(init{3},o);
% end;
% e(:,:,3)=(retdiag(-beta(:))*((repmat(oo,length(init{2}),1)).^2)).*e(:,:,1);
% end;
% end;
% 

% MODIF k0********
% sqrt_k0=repmat(sqrt(init{3}),1,length(init{2}));
% e(:,:,1)=retdiag(sqrt_k0)*e(:,:,1);e(:,:,2)=retdiag(1./sqrt_k0)*e(:,:,2);e(:,:,3)=retdiag(1./sqrt_k0)*e(:,:,3); % normalisation a flux de poynting=1
...
Epa=zeros(numel(R),3);Mua=zeros(numel(R),3);
for ii=1:length(r)-1;jj=find((R>=r(ii+1)) & (R<r(ii)));
    Ep(jj)=ep(4,ii);Epa(jj,1)=ep(4,ii);Epa(jj,2)=ep(6,ii);Epa(jj,2)=ep(6,ii);
    Mu(jj)=ep(1,ii);Mua(jj,1)=ep(1,ii);Mua(jj,2)=ep(2,ii);Mua(jj,3)=ep(3,ii);
end;
Ep_0=ep(4,end);Mu_0=ep(1,end);
[e,o]=deal(zeros(numel(R),6));
%for ii=1:3;o(:,ii)=Mu;o(:,ii+3)=Ep;end;
for ii=1:3;o(:,ii)=Mua(:,ii);o(:,ii+3)=Epa(:,ii);end;
ep=p(1:N);
em=p(N+1:2*N);
hp=p(2*N+1:3*N);
hm=p(3*N+1:4*N);
...
xxx=[x;xxx];
prv=rettsearch(xxx(:,1),xxx(:,2),mmaille,xx,yy);
...
H1_E(isnan(H1_E))=0;H1_H(isnan(H1_H))=0;% modif 6 2017 pour eviter les nan
