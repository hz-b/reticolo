function [e,Z,o,w,PP,P,p,X,Y,wxy,Flux_Poynting]=res3(X,Y,aa,profil,einc,parm);
...
if length(parm.res3.pertes_poynting)<num_tranches;parm.res3.pertes_poynting=[parm.res3.pertes_poynting,repmat(parm.res3.pertes_poynting(end),1,num_tranches-length(parm.res3.pertes_poynting))];end;
...
cale=1:6;if cal_pertes;calo=4:6;else;calo=4;end;% modif pour calo 10 2020
% recherche des points de discontinuité 
x_disc=[];y_disc=[];
for ii=1:size(tab,1);
...
ttab=tab;if size(ttab,2)==3;ttab(:,4)=1;end;
if cal_pertes & ~all(parm.res3.pertes_poynting==0);% on complete tab par des plans de hauteur 0 pour calculer Poynting
tab=zeros(2*size(ttab,1)+1,4);
for ii=1:size(ttab,1);tab(2*ii-1,:)=[0,ttab(ii,2),1,1];tab(2*ii,:)=ttab(ii,:);
if parm.res3.pertes_poynting(ii)==1;tab(2*ii,3:4)=0;end;% pour gain de temps
end;
tab(end,:)=[0,ttab(end,2),1,1];
end;

cale=cale+parm.res3.apod_champ*1i;
...
% if size(o,4)<3;% pour les textures anisotropes generales
% p=.5*(abs(e(:,:,:,1)).^2+abs(e(:,:,:,2)).^2+abs(e(:,:,:,3)).^2).*imag(o(:,:,:));0;
% else;
p=.5*(imag(o(:,:,:,1)).*abs(e(:,:,:,1)).^2+imag(o(:,:,:,2)).*abs(e(:,:,:,2)).^2+imag(o(:,:,:,3)).*abs(e(:,:,:,3)).^2);
%end;
p(isnan(p))=0;

P=reshape(p,length(z),[])*wxy(:);
o=o(:,:,:,1);
o=sqrt(o);% indice ne pas faire o=sqrt(o(:,:,:,1);% bugg
          % pour compatibilite avec V8
X=x*UN;Y=y*UN;
end;
P=P.';
w=w*UN; 
...
cal_Poynting=~all(parm.res3.pertes_poynting==0);
if (conique | init{end}.dim==1) & nargout>9;cal_Poynting=1;end 
if init{end}.dim==2 & nargout>10;cal_Poynting=1;end 

if cal_Poynting;% Poynting**********
Z_strates=[0;cumsum(flipud(ttab(:,1)))];
num_strates=zeros(size(Z_strates));
for ii=1:length(num_strates);[prv,num_strates(ii)]=min(abs(z-Z_strates(ii)));end;
if init{end}.dim==1;  % 1D
Flux_Poynting=UN*retpoynting(e(num_strates,:,:),[0,-1],wx,[]);
else;% 2D
Flux_Poynting=UN*retpoynting(e(num_strates,:,:,:),[0,0,-1],wx,wy,[]);
end
PP=diff(Flux_Poynting);
PP=flipud(PP(:)).';% pertes par couche de haut en bas;
end;                                 % Poynting**********

zlim=[flipud(cumsum(flipud(ttab(:,1))));0];
%for ii=1:length(f_tranches);if parm.res3.pertes_poynting(f_tranches(ii))==0;f=find(z<zlim(ii) & z>zlim(ii+1));PP(f_tranches(ii))=sum(P(f).*w(f));end;end;;
for ii=1:length(zlim)-1;if parm.res3.pertes_poynting(ii)==0;
		f=find(z<zlim(ii) & z>zlim(ii+1));PP(ii)=sum(P(f).*w(f));
	end;end;;
end; % calcul des pertes
...
if cal_Poynting;
Flux_Poynting=-fliplr(Flux_Poynting);%de haut en bas dirige vers le haut
if (conique | init{end}.dim==1) & nargout>9;wxy=Flux_Poynting;end 
end;
end;
