...
persistent bibli;  % utilisation d'une bibliotheque (obsolate)
...
if nargin<3;c=0;end;c_st=c;if length(c)>1;xsource=c(2:end);c=c(1);else xsource=[];end;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

if ~isstruct(init{end}); % 0 D rapide ou yeh

if length(init)==2;a=retyeh_GNV('retcouche',w,init{1});return;end; % yeh 
if ischar(init{end});a=retcouche_NonLocal(init,w,c_st);return;end;% 0D non local
...
[prv1,prv2,prv3,prv4,a]=retabeles(init{1},w(:,2),w(:,1),init{2},init{3},[],init{4});
...
if ~isempty(init{end}.L);if nargin>3;c=c+i;end;% cylindrique_radial
...
sc=zeros(n1,n1);cs=zeros(n1,n1);scm=zeros(n1,n1);csm=zeros(n1,n1);% modif 4 2017
alpha=zeros(0,2);
...
sc(m,:)=reshape(sx.'*c,1,n1);cs(m,:)=reshape(cx.'*s,1,n1);
scm(m,:)=reshape(sxm.'*cm,1,n1);csm(m,:)=reshape(cxm.'*sm,1,n1);
...
if size(eep,3)==18;% anisotropie generale 2015
ep=reshape(eep,size(ep));	
else;
for ii=1:2;
