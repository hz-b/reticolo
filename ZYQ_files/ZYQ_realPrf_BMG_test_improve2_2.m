...
%profile from AFM
Prf=importdata('OneGroove_600EMIL.txt');
Prf=Prf*1E3; % m to nm
...
x_L=Prf(end,1); x_step=1; x=linspace(0,x_L,round(x_L/x_step)+1);%
...
for theta_0=min(thetaEst-0.12,89.35):0.005:min(thetaEst+0.1,89.95)
    theta_0  
    nn=GR_order+3;% ordres de fourier
...
%         end
        ...
        if te_tm==1
            te=[te,result.inc_top_reflected.efficiency{-GR_order}];
            theta=[theta,theta_0];
        else
            tm=[tm,result.inc_top_reflected.efficiency{-GR_order}];
            avg_tetm=(te+tm)./2;
