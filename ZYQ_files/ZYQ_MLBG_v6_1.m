...
LD=1000/(grPeriod_lpermm)*1000*GR_groove;
thetaEst=(ThetaBRAGG-asind(GR_order*GR_groove*1.2398/LD/(photonEnergy_eV/1000)/2/sind(ThetaBRAGG))) ;


end

function [ef,loss_per_layer] = efficiency_bgrML(grPeriod_lpermm, GR_groove,...
...
groove=Prf(2:end,:);
groove_i=1;
while groove_i<GR_groove
    groove(:,1)=groove(:,1)+p_nm;
    Prf=[Prf;groove];
    groove_i=groove_i+1;
end
x=linspace(0,p_nm*GR_groove,round(p_nm/x_resolution_nm)+1);%
...
% initialisation
parm.res1.trace= 0;
aa = res1(lambda_nm, p_nm*GR_groove, textures, nn, k_parallel, parm);
...
metadata.ML_d_nm=3.7;
metadata.ML_d_HZtod=0.4;
metadata.ML_N=10;
...
metadata.GR_order=4;
metadata.photonEnergy_eV=[2500, 3000];
% metadata.FourierOrders=metadata.GR_order*metadata.GR_groove; %or FourierOrders=GR_order*GR_groove+3;
...
if numLoops==0
    ...
    metadataCurrent.grBA_deg=1.93;
    metadataCurrent.grAntiBA_deg=metadataCurrent.grBA_deg*3;
   metadataCurrent.FourierOrders=metadata.GR_order*metadata.GR_groove; %or FourierOrders=GR_order*GR_groove+3;

    grazing_angle_deg=thetaEst; %(thetaEst-0.06*2:0.005:thetaEst+0.06*2)
        [ef,loss_per_layer] = efficiency_bgrML(metadataCurrent.grPeriod_lpermm,metadataCurrent.GR_groove, ...
        ...
        idx_DesignOrder=find(metadataCurrent.FourierOrders:-1:0==metadataCurrent.GR_order*metadataCurrent.GR_groove);
...
while  k>0 && loopIndices{1} <= loopBounds(1)
    ...
    metadataCurrent.grBA_deg=1.93;
    metadataCurrent.grAntiBA_deg=metadataCurrent.grBA_deg*3;
    metadataCurrent.FourierOrders=metadata.GR_order*metadataCurrent.GR_groove+(metadataCurrent.GR_groove-1)*0; %or FourierOrders=GR_order*GR_groove+3;
    ...
    i=1;
    for grazing_angle_deg=thetaEst-0.06:0.01:thetaEst+0.06*2 %2.021 %(
        ef = efficiency_bgrML(metadataCurrent.grPeriod_lpermm,metadataCurrent.GR_groove, ...
            metadataCurrent.grBA_deg, metadataCurrent.grAntiBA_deg, metadataCurrent.photonEnergy_eV, grazing_angle_deg, ...
            metadataCurrent.material_sub,metadataCurrent.material_HZ,metadataCurrent.material_LZ, ...
            metadataCurrent.ML_d_nm,metadataCurrent.ML_d_HZtod,metadataCurrent.ML_N,...
            metadataCurrent.z_resolution_nm,metadataCurrent.x_resolution_nm, metadataCurrent.FourierOrders,metadataCurrent.plSelect);
    ...
    plot(theta,eff);xlabel('theta');title('Diffraction efficiency');legend('TE');ylabel('-4th diffraction efficiency');pause(eps);            
    hold on    
...
%     save(['bgrCmp_effAll_v5_02_',num2str(metadata.grPeriod_lpermm),'LinePerMm.mat'],'outputdata0');
%     save(['bgrCmp_effAll_v5_02_1_',num2str(metadata.grPeriod_lpermm),'LinePerMm.mat'],'outputdata1');
%  
