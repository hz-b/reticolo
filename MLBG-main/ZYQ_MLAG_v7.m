...
metadata.grWidthtoD=0.35;
metadata.grDepth_nm=4;
metadata.grTrapezoidAng1_deg=10;
metadata.grTrapezoidAng2_deg=60; %please choose both = 90 deg(ideal case) or both~=90deg
...
if numLoops==0
    ...
    grazing_angle_deg=thetaEst; %(thetaEst-0.06*2:0.005:thetaEst+0.06*2)
        ef = efficiency_agrML(metadataCurrent.grPeriod_lpermm, ...
            metadataCurrent.grWidthtoD, metadataCurrent.grDepth_nm, metadata.grTrapezoidAng1_deg,metadata.grTrapezoidAng2_deg,...
            metadataCurrent.photonEnergy_eV, grazing_angle_deg, ...
...
while  k>0 && loopIndices{1} <= loopBounds(1)
    ...
    i=1;
    for grazing_angle_deg=(thetaEst-0.06:0.005:thetaEst+0.06*2)
        ef = efficiency_agrML(metadataCurrent.grPeriod_lpermm, ...
            metadataCurrent.grWidthtoD, metadataCurrent.grDepth_nm, metadata.grTrapezoidAng1_deg,metadata.grTrapezoidAng2_deg,...
            metadataCurrent.photonEnergy_eV, grazing_angle_deg, ...
            metadataCurrent.material_sub,metadataCurrent.material_HZ,metadataCurrent.material_LZ, ...
...
%   r  save(['bgrCmp_effAll_v5_02_',num2str(metadata.grPeriod_lpermm),'LinePerMm.mat'],'outputdata0');
...
function ef = efficiency_agrML(grPeriod_lpermm, ...
                grWidthtoD, grDepth_nm,  grTrapezoidAng1_deg, grTrapezoidAng2_deg,...
                photonEnergy_eV, grazing_angle_deg, ...
...
th_nm = grDepth_nm+z_resolution_nm*25+ML_d_nm*ML_N;

% generate texture and Profile
if 1/tand(grTrapezoidAng1_deg) && 1/tand(grTrapezoidAng2_deg)==0
    edge_z = grDepth_nm;
    edge_x = grWidthtoD*p_nm;
else
    pos1 = [(p_nm-grWidthtoD*p_nm)/2-grDepth_nm/tand(grTrapezoidAng1_deg),0];
    pos2 = [(p_nm-grWidthtoD*p_nm)/2,grDepth_nm];
    pos3 = [(p_nm+grWidthtoD*p_nm)/2,grDepth_nm];
    pos4 = [(p_nm+grWidthtoD*p_nm)/2+grDepth_nm/tand(grTrapezoidAng2_deg),0];
