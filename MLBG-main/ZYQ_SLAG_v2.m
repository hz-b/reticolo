...
for grPeriod_lpermm=800 %[1000,500,150:-20:20]
    effAll=[];
    figure
    for FourierOrders=5
        for energy=380
            photonEnergy_eV= energy;
            %              [eta,ef,loss_per_layer] = efficiency_bgrSL(grPeriod_lpermm, ...
            %                 grDepth_nm, grRatio, photonEnergy_eV, grazing_angle_deg, material, z_resolution_nm,x_resolution_nm, FourierOrders,plSelect);
            %
            [eta,ef] = efficiency_bgrSL(grPeriod_lpermm, ...
                grDepth_nm, grRatio, photonEnergy_eV, grazing_angle_deg, material, z_resolution_nm,x_resolution_nm, FourierOrders,plSelect);
...
%         plot(50:10:1000,eff,'-','DisplayName',['Photon energy: ',num2str(energy)]);xlabel('Harmonics');ylabel('1 order efficiency''ordre 0');grid;title('TEST RETICOLO');pause(eps);
%         hold on
        effAll=[effAll;eff];
        eff=[];
        energy_=[];
...
%     save(['agrCmp_effAll_v1_',num2str(grPeriod_lpermm),'LinePerMm_Depth60_Harm100_200.mat'],'effAll');
end
