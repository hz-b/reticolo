...
metadata.GR_order=1;
thetaEst=estimateTheta(metadata.material_HZ,metadata.material_LZ, ...
    metadata.ML_order, metadata.ML_d_nm,metadata.ML_d_HZtod, ...
    metadata.GR_order,metadata.GR_groove,metadata.grPeriod_lpermm,metadata.photonEnergy_eV);

eff=[];
i=1;
outputData=zeros(length((thetaEst-0.2:0.01:thetaEst+0.2)),metadata.FourierOrders*2+2+1);

% Start the timer
tic;
for energy=6000:100:8500
    metadata.photonEnergy_eV=energy;
for grazing_angle_deg=(thetaEst-0.05:0.01:thetaEst+0.05)
ef = efficiency_bgrML(metadata.grPeriod_lpermm, ...
                metadata.grBA_deg, metadata.grAntiBA_deg, metadata.photonEnergy_eV, grazing_angle_deg, ...
                metadata.material_sub,metadata.material_HZ,metadata.material_LZ, ...
                metadata.ML_d_nm,metadata.ML_d_HZtod,metadata.ML_N,...
                metadata.z_resolution_nm,metadata.x_resolution_nm, metadata.FourierOrders,metadata.plSelect);
            outputData(i,1)=grazing_angle_deg;
            outputData(i,((1:metadata.FourierOrders+1))*2)=ef.inc_top_reflected.efficiency;
            outputData(i,((1:metadata.FourierOrders+1))*2+1)=90-ef.inc_top_reflected.theta;
 outputDatawithMetaData={metadata,outputData};

eff(i)=ef.inc_top_reflected.efficiency(5);
...
save(['bgrCmp_effAll_v5_',num2str(metadata.grPeriod_lpermm),'LinePerMm.mat'],'outputDatawithMetaData');
elapsedTime = toc;
disp(['Simulation time at energy ' num2str(energy) ': ' datestr(seconds(elapsedTime), 'HH:MM:SS')]);
end
